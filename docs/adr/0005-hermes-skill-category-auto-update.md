...
`hermes update` calls `sync_skills(quiet=True)` and `seed_profile_skills(p.path, quiet=True)` which copy these into every profile's `~/.hermes/skills/` automatically, with hash-based skip on user-modified files. The weekly orchestrator therefore does **not** separately touch these directories — running `hermes update` once in the Sunday self-update cron is enough. This is implemented (not just decided): ADR 0006 makes Hermes the first step of the single `weekly-update-all` pipeline and isolates that step in a transient systemd worker, so the bundle ships before the next component upgrades.
